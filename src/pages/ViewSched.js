...
const ViewSched = ({ scheduleData: propScheduleData, onBack, onEdit, onAddMinutes }) => {
    ...
    const handleAddMinutes = () => {
        ...
        // Check if a minute record already exists for this schedule (by matching matter/title)
        const alreadyExists = allMinutes.find(doc => doc.matter === scheduleData.title || doc.hearingTitle === scheduleData.title);
        
        if (alreadyExists) {
            toast.warning("A minute record for this hearing already exists. Redirecting...");
            setTimeout(() => navigate(`/minutes-info/${alreadyExists.id}`), 1000);
            return;
        }

        // No existing record - create a new one by navigating to 'new'
        if (onAddMinutes) {
            onAddMinutes();
        }
    };
