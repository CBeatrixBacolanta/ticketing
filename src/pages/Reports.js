...
const Reports = ({ user }) => {
  ...
  useEffect(() => {
    ...
    // 3. Map through hearings and count only those that are 'done'
    savedHearings.forEach(hearing => {
      const hearingDate = new Date(hearing.date);
      const hearingMonth = hearingDate.toLocaleString('default', { month: 'long' });
      const hearingYear = hearingDate.getFullYear();
      const hearingDay = hearingDate.getDate();

      // Check if hearing matches selected filters AND is marked as done
      if (
        hearingMonth === selectedMonth && 
        hearingYear === parseInt(selectedYear) &&
        (hearing.status?.toLowerCase() === 'done' || hearing.status?.toLowerCase() === 'completed')
      ) {
        // Increment the count for that specific day
        if (dailyCounts[hearingDay - 1]) {
          dailyCounts[hearingDay - 1].clients += 1;
        }
      }
    });

    setReportData(dailyCounts);
  }, [selectedMonth, selectedYear]); // Recalculate when filters change
          ...
          <div className="graph-section-wrapper">
            <ResponsiveContainer width="100%" height={400}>
              <BarChart data={reportData} margin={{ top: 30, right: 10, left: 0, bottom: 0 }}>
                ...
                <YAxis domain={[0, 'auto']} axisLine={false} tickLine={false} width={40} />
